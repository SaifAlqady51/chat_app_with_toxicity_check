...
const initialMessages = ["Hello", "What's up"]

export default function Home() {
  const [messages,setMessages] = useState<string[] | null>(initialMessages)
  ...
  const sendMessage = (event:React.FormEvent<HTMLFormElement>) => {
    ...
    return (
        <div className="flex flex-col justify-center items-center color">
            <h1 className="flex text-6xl pt-5">Check toxic messages</h1>
            <form className="relative flex h-10 w-full min-w-[200px] max-w-[36rem] m-5" onSubmit={sendMessage}>
                <input
                    type="text"
            ...
            <ul className='flex flex-col'>
              {messages?.map(message => {
                return (<li key={messages.indexOf(message)}>{message}</li>)})
              }
            </ul>
        </div>
    );
